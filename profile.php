...
<div class="right">
	...
	<div class="bkb"></div>
			...
			<?php
						...
						$id = $_SESSION['SESS_MEMBER_ID'];
						mysqli_query($connection,"CREATE TEMPORARY TABLE IF NOT EXISTS inbox AS (select member_id from friends where friends_with='$id')")or die(mysqli_error($connection));
						mysqli_query($connection,"insert into inbox select friends_with from friends where member_id='$id' ")or die(mysqli_error($connection));
						$post = mysqli_query($connection,"SELECT * FROM members WHERE member_id not in(select * from inbox) and member_id!='$id' LIMIT 0,3")or die(mysqli_error($connection));
						...
						mysqli_query($connection,"drop table inbox")or die(mysqli_error($connection));
					?>
...
mysqli_query($connection,"CREATE TEMPORARY TABLE IF NOT EXISTS inbox AS (select member_id from friends where friends_with='$id' and status='conf')")or die(mysqli_error($connection));
mysqli_query($connection,"insert into inbox select friends_with from friends where member_id='$id' and status='conf' ")or die(mysqli_error($connection));
mysqli_query($connection,"insert into inbox select member_id from members where member_id='$id' ")or die(mysqli_error($connection));

 $query  = "SELECT *,UNIX_TIMESTAMP() - date_created AS TimeSpent FROM comment WHERE member_id in (select * from inbox) ORDER BY comment_id DESC";
...
while($row = mysqli_fetch_assoc($result))
{
   echo '<div class="information">';
	...
	//echo $idd;
	$result1 = mysqli_query($connection,"SELECT * FROM members WHERE member_id='$idd' ");
	...
	if ($row1["member_id"]==$id){
	
	echo'<a class="style" href="deletepost1.php?id=' . $row["comment_id"] . '">delete</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="style" href="like.php?id='.$row["comment_id"].'&amp;user_id='.$id.'"><img width=20 height=20  src=img/like.png>Like</a>';
	echo '<font color="Red">(' . $numres2. ')</font>&nbsp;&nbsp;&nbsp';
	}
	else {
		echo'<a class="style" href="deletepost1.php?id=' . $row["comment_id"] . '"></a>&nbsp;&nbsp;<a class="style" href="like.php?id='.$row["comment_id"].'&amp;user_id='.$id.'"><img width=20 height=20  src=img/like.png>Like</a>';
		echo '<font color="red">(' . $numres2. ')</font>&nbsp;&nbsp;&nbsp';
	}
