...
<?php
									...
									$member_id = $_SESSION['SESS_MEMBER_ID'];
									$post = mysqli_query($connection,"SELECT * FROM messages WHERE recipient = '$id' ORDER by datetime DESC")or die(mysqli_error($connection));
									while($row = mysqli_fetch_array($post)){
										$id = $row['receiver'];
										$hu_u = mysqli_query($connection,"SELECT * FROM members WHERE member_id = '$id'")or die(mysqli_error($connection));
										$rows = mysqli_fetch_array($hu_u);
										$iyaid = $row['message_id'];
										echo'<div class="information">';
										echo'<hr width=640>
										</br>';
											$sql=mysqli_query($connection,"SELECT * FROM members WHERE member_id='$id'") or die(mysqli_error($connection));
